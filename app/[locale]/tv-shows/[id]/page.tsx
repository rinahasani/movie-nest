...
export default function TvShowDetailsPage({
  ...
  useEffect(() => {
    const fetchTvShow = async () => {
      try {
        ...
        const fetchedTvShow = await getTvShowDetails(tvShowId, locale);
        setTvShow(fetchedTvShow);
        if (fetchedTvShow.seasons && fetchedTvShow.seasons.length > 0) {
          const firstValidSeason = fetchedTvShow.seasons.find(
  ...
  const handleSeasonChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const season = tvShow.seasons?.find(
      (s: Season) => s.season_number === seasonNumber
    );
  ...
  return (
    <div className="relative text-white min-h-screen bg-black">
      ...
      <div className="relative z-10 container mx-auto p-8 py-12">
        <div className="flex flex-col md:flex-row items-center md:items-center gap-8 bg-gradient-to-l from-black/30 to-black/80 p-6 rounded-lg shadow-xl">

...
export const API_ENDPOINTS = {
  DISCOVER: (page = 1, language = "en-EN") =>
    `${BASE_URL}/discover/movie?page=${page}&api_key=${API_KEY}&language=${language}&include_adult=false`,
  GENRES: (language = "en-EN") =>
    `${BASE_URL}/genre/movie/list?api_key=${API_KEY}&language=${language}`,
  MOVIE_DETAILS: (movieId, language = "en-EN") =>
    `${BASE_URL}/movie/${movieId}?api_key=${API_KEY}&language=${language}&include_adult=false`,
  MOVIE_BY_GENRE: (page = 1, language = "en-EN", genreId) =>
  ...
  SEARCH_TV: (query, language = "en-EN") =>
    `${BASE_URL}/search/tv?query=${encodeURIComponent(query)}&api_key=${API_KEY}&language=${language}`
};
